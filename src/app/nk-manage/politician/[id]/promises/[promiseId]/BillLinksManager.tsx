...
export function BillLinksManager({
  ...
  async function handleAutoMatch() {
    ...
    try {
      const res = await fetch(`/api/nk-manage/promises/${promiseId}/auto-match`, {
        method: "POST",
      });
      if (res.ok) {
        await refreshLinks();
  ...
  async function refreshLinks() {
    const res = await fetch(`/api/nk-manage/promises/${promiseId}/bill-links`);
    if (res.ok) {
      const data = await res.json();
  ...
  async function handleToggleAlignment(linkId: string, current: string) {
    ...
    const res = await fetch(
      `/api/nk-manage/promises/${promiseId}/bill-links/${linkId}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
